...
## BUG-03 — Product Prices Are Unstable and Do Not Match Cart Prices for visual_user
...
## BUG-04 — Selecting a Product Opens Details for a Different Product for problem_user
...
Additional exploratory testing with `problem_user` showed broader product-catalog inconsistencies, including incorrect product images, mismatched product details, and a `Product Not Found` result for one product.
...
## BUG-05 — Remove Button Does Not Remove Product from Product Details Page for problem_user
...
Removing the same product from the Cart page works successfully. The equivalent removal action also worked correctly when tested with `standard_user`.
...
## BUG-06 — Header Elements Are Misaligned for visual_user
...
3. Compare their positioning with the same page when using `standard_user`.
...
For `visual_user`, header elements are shifted from their normal positions and the cart icon appears unusually close to the product sorting control. The layout difference persists after refreshing the page and differs from the layout displayed for `standard_user`.
...
`locked_out_user` was unable to log in and consistently received:
...
`performance_glitch_user` consistently loaded the application noticeably slower than `standard_user`.
...
Because the account is explicitly named `performance_glitch_user`, the behavior was documented as an observation rather than reported as an unexpected defect.
...
Chrome DevTools inspection showed that the text is contained within a footer `<div>` and is not implemented using `<a>` elements or `href` attributes. The text was not interactive.

No requirement confirming that these labels must be links was available, so this was recorded as an observation/product suggestion rather than a confirmed defect.
